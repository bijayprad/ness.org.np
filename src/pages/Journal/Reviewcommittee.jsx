export default function Reviewcommittee() {
  return (
    <div>
      <h2 className="text-3xl font-bold mb-4">Review Committee</h2>
      <p className="mb-4">
        The Review Committee of distinguished researchers and experts in the field of Statistics and Data Science.
        They oversee the quality, integrity, and academic rigor of all journal publications.
      </p>
      <ul className="list-disc ml-6">
<li>Prof. Dr. Bijay Lal Pradhan – Tribhuvan University</li>
<li>Prof. Dr. Ram Prasad Khatiwada – Tribhuvan University</li>
<li>Prof. Dr. Vikash Raj Satyal – Tribhuvan University</li>
<li>Dr. Ananta Raj Dhungana – Pokhara University</li>
<li>Dr. Niranjan Shrestha  – Pokhara University</li>
<li>Dr. Shyam Kishore Mahato – Professor, Rajarshi Janak University</li>
<li>Dr. Dharmendra Kumar Singh –Associate Professor, Purbanchal University</li>
<li>Dr. Samir KC – Professor, Wittgenstein Centre </li>
<li>Dr. Amita Pradhan – Professor </li>


      </ul>
      <p className="mt-4">
        For any editorial inquiries, please contact bijayprad@gmail.com.
      </p>
    </div>
  );
}
